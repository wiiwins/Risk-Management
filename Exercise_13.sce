...
// value at risk
subplot(3,1,1);
plot (td+2:length(s),VaR_historic(1,td+2:$));
plot (td+2:length(s),VaR_lognorm(1,td+2:$), 'r');
title ('Value at Risk at 90%');
xlabel('trading day');
legend("Historical Simulation", "log_normal Method");

subplot(3,1,2);
plot (td+2:length(s),VaR_historic(2,td+2:$),'black');
plot (td+2:length(s),VaR_lognorm(2,td+2:$), 'r');
...
// expected shortfall
subplot(3,1,3);
plot (td+2:length(s),ES_historic(1,td+2:$));
plot (td+2:length(s),ES_historic(2,td+2:$), 'g');
